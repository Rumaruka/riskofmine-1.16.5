...
public class CommonChestFeature<C> extends Feature<CommonChestFeatureConfig> {
    public CommonChestFeature(Codec<CommonChestFeatureConfig> codec) {
        super(codec);
    }


    @Override
    public boolean place(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, CommonChestFeatureConfig config) {
        ...
        for (Integer integer : list) {
            for (Integer integer1 : list1) {
                ...
                if (reader.isEmptyBlock(blockpos) || reader.getBlockState(blockpos).getCollisionShape(reader, blockpos).isEmpty()) {
                    reader.setBlock(blockpos, ROMBlocks.SMALL_CHEST.defaultBlockState(), 2);
                    LockableLootTileEntity.setLootTable(reader, rand, blockpos, ROMLootTables.SMALL_CHEST);
