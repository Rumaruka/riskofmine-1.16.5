...
public class LargeChestFeature<C> extends Feature<LargeChestFeatureConfig> {
    public LargeChestFeature(Codec<LargeChestFeatureConfig> codec) {
        super(codec);
    }


    @Override
    public boolean place(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, LargeChestFeatureConfig config) {
        ...
        for (Integer integer : list) {
            for (Integer integer1 : list1) {
                ...
                if (reader.isEmptyBlock(blockpos) || reader.getBlockState(blockpos).getCollisionShape(reader, blockpos).isEmpty()) {
                    reader.setBlock(blockpos, ROMBlocks.LARGE_CHEST.defaultBlockState(), 2);
                    LockableLootTileEntity.setLootTable(reader, rand, blockpos, ROMLootTables.LARGE_CHEST);
