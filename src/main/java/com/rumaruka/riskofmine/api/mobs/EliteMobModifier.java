package com.rumaruka.riskofmine.api.mobs;
...
public abstract class EliteMobModifier {
     ...
     */
    protected EliteMobModifier nextMod;
     ...
     */
    protected EliteMobModifier(EliteMobModifier nxtMod) {
        this();
        this.nextMod = nxtMod;
    }
    ...
    public void onSpawningCompleteStoreMods(LivingEntity entity) {
        ...
        if (!oldTag.isEmpty() && !oldTag.equals(getLinkedModNameUntranslated())) {
            RiskOfMine.logger.info("Infernal Mobs tag mismatch!! Was [{}}], now trying to set [{}}] \n", oldTag, getLinkedModNameUntranslated());
        }
    ...
    public float onHurt(LivingEntity mob, DamageSource source, float amount) {
        if (nextMod != null) {
            ...
            if (source.getEntity().level.isClientSide && source.getEntity() instanceof PlayerEntity) {
    ...
    public boolean equals(Object o) {
        return (o instanceof EliteMobModifier && ((EliteMobModifier) o).getModName().equals(getModName()));
    }
    ...
    public int getModSize() {
        if (bufferedSize == 0) {
            bufferedSize = 1;
            EliteMobModifier nextmod = this.nextMod;
            while (nextmod != null) {
                bufferedSize++;
    ...
    public String getEntityDisplayName(LivingEntity target) {
        if (bufferedEntityName == null) {
            ...
            String prefix = size <= 5 ? TextFormatting.AQUA + I18n.get("translation.riskofmine:rareClass")
                    : size <= 10 ? TextFormatting.YELLOW + I18n.get("translation.riskofmine:ultraClass") : TextFormatting.GOLD + I18n.get("translation.riskofmine:eliteClass");
